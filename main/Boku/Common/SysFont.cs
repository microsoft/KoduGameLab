...
namespace Boku.Common
{
    ...
    public static partial class SysFont
    {
        ...
        public static void Init()
        {
            ...
            // Minimize memory use if SpriteFont is preferred.  We still
            // need SysFont for some things. (CardFace, etc.)
            if (!BokuSettings.Settings.UseSystemFontRendering)
            {
                width = 256;
                height = 64;

                CacheEntry.MaxEntries = 2;
            }
            ...
            texture = new Texture2D(BokuGame.bokuGame.GraphicsDevice, width, height);
        ...
        public static void EndBatch()
        {
            ...
            if (entries.Count > 0)
            {
                ...
                foreach (BatchEntry entry in entries)
                {
                    ...
                    entry.clipRect = new System.Drawing.Rectangle((int)entry.position.X, (int)entry.position.Y, w, h);
                    */

                    // Grow batchRect to include this entry.
                    batchRectF = System.Drawing.RectangleF.Union(batchRectF, entry.clipRect);
        ...
        static void CopyBitmapToTexture(Rectangle batchRect, Rectangle bitmapRect, Bitmap bitmap, Texture2D texture, Color[] textureData, out Microsoft.Xna.Framework.Rectangle targetRect)
        {
            ...
            {
                batch.Draw(UI2D.Shared.BlackButtonTexture, new Vector2(-1000, -1000), Color.White);
            }
            batch.End();
        ...
        public static SystemFont GetSystemFont(string familyName, float emSize, FontStyle style)
        {
            ...
            if (!systemFonts.TryGetValue(systemFontKey, out systemFont))
            {
                ...
                {
                    // Saw a case in the logs where family "Calibri" didn't support
                    // style "Regular".  So, attempt to get a font without specifying
                    // the style instead.
                    systemFont = new SystemFont(familyName, emSize);
                }
