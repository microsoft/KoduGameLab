...
namespace KoiX
{
    ...
    class MyMath
    {
        ...
        public static Rectangle Lerp(Rectangle a, Rectangle b, float t)
        {
            ...
            result.Location = new Point((int)(a.Location.X * (1 - t) + b.Location.X * t), (int)(a.Location.Y * (1 - t) + b.Location.Y * t));
            result.Width = (int)(a.Width * (1 - t) + b.Width * t);
            result.Height = (int)(a.Height * (1 - t) + b.Height * t);
        ...
        public static RectangleF Lerp(RectangleF a, RectangleF b, float t)
        {
            ...
            result.Position = a.Position * (1 - t) + b.Position * t;
            result.Size = a.Size * (1 - t) + b.Size * t;

            return result;
        }
        public static Point Lerp(Point a, Point b, float t)
        {
            Point result = new Point((int)(a.X * (1 - t) + b.X * t), (int)(a.Y * (1 - t) + b.Y * t));

            return result;
        }
        public static Padding Lerp(Padding a, Padding b, float t)
        {
            Padding result = new Padding((int)(a.left * (1 - t) + b.left * t),
                                        (int)(a.top * (1 - t) + b.top * t),
                                        (int)(a.right * (1 - t) + b.right * t),
                                        (int)(a.bottom * (1 - t) + b.bottom * t));
        ...
        /// Returns angle in radians from the given direction vector.
        /// </summary>
        /// <param name="dir">Direction vector.  Does not need to be normalized.</param>
        /// <returns></returns>
        public static float RotationFromDirection(Vector2 dir)
        {
            float angle = 0.0f;
        ...
        /// <summary>
        /// Returns angle in radians from the given direction vector.
        /// Return value is in range (-pi, pi]
        /// </summary>
        /// <param name="dir">Direction vector.  Does not need to be normalized.</param>
        /// <returns></returns>
        public static float AngleFromVector(Vector2 dir)
        {
            ...
            return angle;
        }   // end of AngleFromVector()

        /// <summary>
        /// Calculates whether or not two line segments intersect.
        /// </summary>
        /// <param name="v0">vertex 0 for line 0</param>
        /// <param name="v0">vertex 1 for line 0</param>
        /// <param name="v1">vertex 0 for line 1</param>
        /// <param name="v2">vertex 1 for line 1</param>
        /// <param name="t">normalized dist along line 0 where intersection takes place, range will be in [0,1] if intersection is valid.</param>
        /// <returns></returns>
        public static bool LineLineIntersect(Vector2 v0, Vector2 v1, Vector2 v2, Vector2 v3, out float t)
        {
            t = 0;
            Vector2 b = v1 - v0;
            Vector2 d = v3 - v2;
            ...
            // NOTE They may be colinear in which case they 
            // actually should count as intersecting.
            ...
            // Calc t.
            t = (cx * d.Y - cy * d.X) / cross;
            // Are we in the extend of line0?
            if (t < 0 || t > 1)
            {
                return false;
            }

            // Calc u.  (Same as t but for line 1)
            float u = (cx * b.Y - cy * b.X) / cross;
            // Are we in range for line 1?
            if (u < 0 || u > 1)
            {
                return false;
            }
            ...
            return true;
        }   // end of LineLineIntersect()

        /// <summary>
        /// Calculates whether two polygons intersect.
        /// The position and normal are approximations and are only
        /// valid if true is returned.  They are always modified.
        /// </summary>
        /// <param name="p0"></param>
        /// <param name="p1"></param>
        ...
        public static bool PolyPolyIntersection(List<Vector2> p0, List<Vector2> p1, out Vector2 position, out Vector2 normal)
        {
            bool result = false;
            ...
            Vector2 int0 = Vector2.Zero;    // First crossing found.
            Vector2 int1 = Vector2.Zero;    // Second crossing found.

            for (int i0 = 0; i0 < p0.Count; i0++)
            {
                for (int i1 = 0; i1 < p1.Count; i1++)
                {
                    float t = 0;
                    if (LineLineIntersect(p0[i0], p0[(i0 + 1) % p0.Count], p1[i1], p1[(i1 + 1) % p1.Count], out t))
                    {
                        if (numIntersections == 0)
                        {
                            int0 = Lerp(p0[i0], p0[(i0 + 1) % p0.Count], t);
                            ++numIntersections;
                        }
                        else if (numIntersections == 1)
                        {
                            int1 = Lerp(p0[i0], p0[(i0 + 1) % p0.Count], t);
                            ++numIntersections;
                            // Force a break out of the double loop.
                            i0 = p0.Count;
                            i1 = p1.Count;
                        }
                    }
            ...
            if (numIntersections != 0)
            {
                // Use midpoint between crossings as intersection point.
                position = (int0 + int1) / 2.0f;

                // Use vector perpendicular to this as normal.
                normal = int0 - int1;
                normal.Normalize();
                // Make perpendicular.
                float tmp = normal.X;
            ...
            return result;
        }   // end of PolyPolyIntersection()

        /// <summary>
        /// Calcs a point in poly test by projecting a horizontal ray
        /// from the point and counting how many edges are crossed.
        /// </summary>
        /// <param name="poly"></param>
        /// <param name="pnt"></param>
        /// <returns></returns>
        public static bool PointInPolygon(List<Vector2> poly, Vector2 pnt)
        {
            bool result = false;
            int i = 0;
            int j = poly.Count - 1;     // 1 less than i but starts wrapped around to end since i==0
