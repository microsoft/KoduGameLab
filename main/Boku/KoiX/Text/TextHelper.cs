...
namespace KoiX.Text
{
    ...
    {
        /// <summary>
        /// Text TextHelper.Justification options.
        /// </summary>
        ...
        public static void ClipStringToWidth(GetFont Wrapper, int width, ref string text)
        {
        ...
        public static void SplitMessage(string inputMessage, int maxLineWidth, GetFont Font, bool preserveCharacterCount, List<string> lines)
        {
            lines.Clear();
        ...
        public static int CalcJustificationOffset(int margin, int width, int textWidth, TextHelper.Justification justification)
        {
            int x = 0;
        ...
        public static bool CharIsValid(int c)
        {
            ...
            // Also reject out of range characters.
            if (c < 0x0020 || c == 65533)
            {
                return result;
            }
        ...
        /// <summary>
        /// Take the text input, breaks it into seperate lines, trims those lines, converts 
        /// any leading colons into spaces (1 colon == 4 spaces), remove any leading or 
        /// trailing empty strings and then recombines the lines and returns the new string.
        ...
        public static string CleanUpString(string str)
        {
            ...
            // Find first and last indices so we can ignore
            // leading and trailing empty strings.
            int firstIndex = 0;
            int lastIndex = strings.Length - 1;

            while (string.IsNullOrEmpty(strings[firstIndex]) && firstIndex < strings.Length - 1)
            {
                ++firstIndex;
            }
            while (lastIndex >= 0 && string.IsNullOrEmpty(strings[lastIndex]))
            {
                --lastIndex;
            }
        ...
        public static void DrawString(GetFont Font, string text, Vector2 pos, Color textColor, Color outlineColor = default(Color), float outlineWidth = 0, int maxWidth = 2048)
        {
            ...
            // is used.
            batch.End();
            blob.RenderText(null, pos, textColor, outlineColor: outlineColor, outlineWidth: outlineWidth);
            batch.Begin();
        ...
        public static void DrawStringNoBatch(GetFont Font, string text, Vector2 pos, Color textColor, Color outlineColor = default(Color), float outlineWidth = 0, bool renderCursor=false, int maxLines = int.MaxValue)
        {
            SpriteBatch batch = KoiLibrary.SpriteBatch;
            ...
            blob.RenderText(null, pos, textColor, outlineColor: outlineColor, outlineWidth: outlineWidth, renderCursor: renderCursor, maxLines: maxLines);

        }   // end of DrawString()

        public static void DrawStringNoBatch(SpriteCamera camera, GetFont Font, string text, Vector2 pos, Color textColor, Color outlineColor = default(Color), float outlineWidth = 0, bool renderCursor = false, int maxLines = int.MaxValue)
        {
            SpriteBatch batch = KoiLibrary.SpriteBatch;
            ...
            blob.RenderText(camera, pos, textColor, outlineColor: outlineColor, outlineWidth: outlineWidth, renderCursor: renderCursor, maxLines: maxLines);

        }   // end of DrawStringNoBatch()

        public static void DrawStringWithShadow(GetFont Font, SpriteBatch batch, Vector2 pos, string text, Color textColor, Color shadowColor, bool invertDropShadow)
        {
        ...
        public static void DrawStringWithShadow(GetFont Font, SpriteBatch batch, int x, int y, string text, Color textColor, Color shadowColor, bool invertDropShadow)
        {
            text = TextHelper.FilterInvalidCharacters(text);
        ...
        /// <returns></returns>
        public static string AddEllipsis(GetFont Font, string input, float maxSize)
        {
