...
namespace Boku.Common
{
    ...
    public partial class TextBlob
    {
        ...
        UIGridElement.Justification justification = UIGridElement.Justification.Left;
        ...
        public int LineSpacingAdjustment
        {
            ...
            set { lineSpacingAdjustment = value; dirty = true; }
        }
        ...
        /// <summary>
        /// Render version that supports drop shadows.
        /// </summary>
        /// <param name="position"></param>
        /// <param name="color"></param>
        /// <param name="shadowColor"></param>
        /// <param name="shadowOffset"></param>
        /// <param name="renderCursor"></param>
        /// <param name="startLine">Line in blob to start rendering.</param>
        /// <param name="maxLines"></param>
        public void RenderWithButtons(Vector2 position, Color color, Color shadowColor = default(Color), Vector2 shadowOffset = default(Vector2), Color outlineColor = default(Color), float outlineWidth = 0, bool renderCursor = false, int startLine = 0, int maxLines = int.MaxValue)
        {
            FlowText();
            ...
            for (int i = startLine; i < Math.Min(lines.Count, startLine + maxLines); i++)
            {
                int lineWidth = 0;
                for (int w = 0; w < lines[i].words.Count; w++)
                {
                ...
                if (justification == UIGridElement.Justification.Center)
                {
                    justificationOffset = (maxWidth - lineWidth) / 2;
                }
                else if (justification == UIGridElement.Justification.Right)
                {
                ...
                for (int w = 0; w < lines[i].words.Count; w++)
                {
                    ...
                    {
                        Texture2D texture = ButtonTexture(word.icon);
                        ssquad.Render(texture, pos, new Vector2(word.width, ButtonSize.Y), "TexturedRegularAlpha");
                        ...
                        if (word.keyIcon == TextHelper.ControlInputs.none)
                        {
                            ...
                            if (KeyFont == UI2D.Shared.GetGameFont30Bold)
                            {
                                KeyFont = UI2D.Shared.GetGameFont24;
                                if (BokuSettings.Settings.UseSystemFontRendering)
                                {
                                    keyFontSize = 24.0f;
                                }
                            }
                            else if (KeyFont == UI2D.Shared.GetGameFont24 || KeyFont == UI2D.Shared.GetGameFont24Bold)
                            {
                                KeyFont = UI2D.Shared.GetGameFont20;
                                if (BokuSettings.Settings.UseSystemFontRendering)
                                {
                                    keyFontSize = 20.0f;
                                }
                            }
                            else if (KeyFont == UI2D.Shared.GetGameFont20)
                            {
                                KeyFont = UI2D.Shared.GetGameFont15_75;
                                if (BokuSettings.Settings.UseSystemFontRendering)
                                {
                                    keyFontSize = 15.75f;
                                }
                            }
                            else if (KeyFont == UI2D.Shared.GetGameFont18Bold)
                            {
                                KeyFont = UI2D.Shared.GetGameFont13_5;
                                if (BokuSettings.Settings.UseSystemFontRendering)
                                {
                                    keyFontSize = 13.5f;
                                }
        ...
        public void FindCursorLineAndPosition(out int curLine, out int xPos)
        {
            ...
            if (lines.Count == 0)
            {
                ...
                else if (justification == UIGridElement.Justification.Center)
                {
                    xPos = Width / 2;
                }
                else if (justification == UIGridElement.Justification.Right)
