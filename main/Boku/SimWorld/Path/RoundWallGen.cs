...
namespace Boku.SimWorld.Path
{
    class RoundWallGen : HiWallGen
    {
        ...
        public override void LoadContent(bool immediate)
        {
            if (diffTex0 == null)
            {
                diffTex0 = KoiLibrary.LoadTexture2D(@"Textures\Terrain\GroundTextures\FREESAMPLES_23");
            }
            if (diffTex1 == null)
            {
                diffTex1 = KoiLibrary.LoadTexture2D(@"Textures\Terrain\GroundTextures\rock2");
            }
            if (normTex0 == null)
            {
                normTex0 = KoiLibrary.LoadTexture2D(@"Textures\Terrain\GroundTextures\FREESAMPLES_23_norm");
            }
            if (normTex1 == null)
            {
    ...
    class RoboWall3 : HiWallGen
    {
        ...
        public override void LoadContent(bool immediate)
        {
            if (diffTex0 == null)
            {
                diffTex0 = KoiLibrary.LoadTexture2D(@"Textures\Terrain\GroundTextures\simple_white");
            }
            if (diffTex1 == null)
            {
                diffTex1 = KoiLibrary.LoadTexture2D(@"Textures\Terrain\GroundTextures\simple_white");
            }
            if (normTex0 == null)
            {
                normTex0 = KoiLibrary.LoadTexture2D(@"Textures\Terrain\GroundTextures\Blue128x128");
            }
            if (normTex1 == null)
            {
