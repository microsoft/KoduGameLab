...
namespace KoiX.Scenes
{
    ...
    /// </summary>
    public class WorldSettingsPage4Scene : BasePageScene
    {
        #region Members
        ...
        List<GUIButtonLabelHelp> guiButtons = new List<GUIButtonLabelHelp>();
        ...
        public WorldSettingsPage4Scene(string nextLabelId = null, string nextLabelText = null, string prevLabelId = null, string prevLabelText = null)
            : base("WorldSettingsPage4Scene", nextLabelId, nextLabelText, prevLabelId, prevLabelText)
        {
            ...
            title = new Label(fullScreenContentDialog, titleFont, theme.LightTextColor, outlineColor: theme.DarkTextColor, outlineWidth: 2.5f, labelId: "worldSettings.buttons");
...

            {
                ...
                {
                    GUIButtonLabelHelp gb = new GUIButtonLabelHelp(fullScreenContentDialog, Font, width, (Classification.Colors)((int)Classification.ColorInfo.First + i), OnChange: null, theme: theme);
                    guiButtons.Add(gb);
                    gb.Margin = new Padding(64, 64, 64, 64);
                }
            ...
            // Add guiButtons to proper rows.
            row1.AddWidget(guiButtons[0]);
            row1.AddWidget(guiButtons[1]);
            row1.AddWidget(guiButtons[2]);
            row2.AddWidget(guiButtons[3]);
            row2.AddWidget(guiButtons[4]);
            row2.AddWidget(guiButtons[5]);
            row3.AddWidget(guiButtons[6]);
            row3.AddWidget(guiButtons[7]);
            row3.AddWidget(guiButtons[8]);
            row4.AddWidget(guiButtons[9]);
            row4.AddWidget(guiButtons[10]);
    ...
    }   // end of class WorldSettingsPage4Scene
