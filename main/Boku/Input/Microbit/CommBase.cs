...
// Code lifted from this now-defunct MSDN article: http://msdn.microsoft.com/en-us/magazine/cc301786.aspx
// It can still be found by searching for the title: "Use P/Invoke to Develop a .NET Base Class Library for Serial Device Communications"
...
namespace Boku.Input
{
    ...
    public abstract class CommBase : IDisposable
    {
        ...
        //Added below to improve robustness of thread start-up.
        ...
        //Added to support the new congestion detection scheme (following two lines):
        private bool[] empty = new bool[1];
        private bool dataQueued = false;

        public bool IsOpen { get { return this.online; } }
        ...
        public class CommBaseSettings
        {
            ...
            //Next two defaults changed to 0 to use new defaulting mechanism dependant on queue size.
            /// <summary>
        ...
        //Added AltName function, PortStatus enum and IsPortAvailable function.
        ...
        public PortStatus IsPortAvailable(string s)
        {
            ...
            if (h == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
            {
                ...
                {
                    //Automatically try AltName if supplied name fails:
        ...
        public bool Open()
        {
            ...
            if (hPort == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
            {
                ...
                {
                    //Try alternative name form if main one fails:
            ...
            //Defaulting mechanism for handshake thresholds - prevents problems of setting specific
        ...
        private void CheckResult()
        {
            ...
            //Fixed a number of problems working with checkSends == false. Byte counting was unreliable because
        ...
        //Deleted OnRing() which was never called: use OnStatusChange instead.
            ...
            try
            {
                while (true)
                {
                    ...
                    if ((eventMask & Win32Com.EV_ERR) != 0)
                    {
                        ...
                        {
                            //BREAK condition has an error flag and and an event flag. Not sure if both
                    ...
                    if ((eventMask & Win32Com.EV_RXCHAR) != 0)
                    {
                        do
                        {
                            ...
                            {
                                //Removed ERROR_IO_PENDING handling as comm timeouts have now
        ...
        private bool CheckOnline()
        {
            ...
            if (online)
            {
                //Avoid use of GetHandleInformation for W98 compatability.
    ...
    public abstract class CommLine : CommBase
    {
        ...
        {
            //Use static encoder for efficiency. Thanks to Prof. Dr. Peter Jesorsky!
        ...
        protected override void OnRxChar(byte ch)
        {
            ...
            {
                //Use static encoder for efficiency.
    ...
    internal class Win32Com
    {
        ...
        internal struct DCB
        {
            ...
            internal void init(bool parity, bool outCTS, bool outDSR, int dtr, bool inDSR, bool txc, bool xOut,
                bool xIn, int rts)
            {
                //Was 0x8001 ans so not setting fAbortOnError
