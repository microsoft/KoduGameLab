...
namespace KoiX
{
    ...
    public class Time
    {
        static double wallClockTotalSeconds = 0.0;      // Total real world seconds since game started.
        static double gameTimeTotalSeconds = 0.0;       // Total game time seconds since game started.
        static double instrumentationTotalSeconds = 0.0; //Total active game seconds since game started
        static float wallClockFrameSeconds = 0.0f;      // Real world time since last call to Update().
        static float gameTimeFrameSeconds = 0.0f;       // Game time seconds since last call to Update().

        static float clockRatio = 1.0f;                 // Ratio of game time / real world.  
        ...
        static float savedRatio = 1.0f;                 // Ratio to return to after a pause.
        static bool paused = false;
        static int frame = 0;                           // Frame counter.
        static int nonPausedFrameCounter = 0;           // Frame count while not paused.  Allows timers to not re-sync when coming out of pause mode.
        static float fps = 60.0f;                       // Current frame rate.
        public static GameTime _gameTime;               // just a copy of the gametime passed by update // *mbm*

        //variables to account for inactivity in the game, used for instrumentation
        static bool activeGameClock;                    // Indicates if the user is active in the game
        public static double inactiveTimeThreshold = 60.0;      // seconds of allowed inactivity before stopping the clock
        static double startInactiveTime = 0.0;          // timer that counts the current number of inactive seconds

        static bool skippingFrames = false;             // On low end machines we can sometimes skip rendering the world
                                                        // to provide better UI responsiveness.
        const double kSkipStartFrameTime = 0.1;         // If frame time is longer than this, start skipping.
        const double kSkipStopFrameTime = 0.07;         // If frame time is shorter than this, stop skipping.
        const int kFrameSkipModValue = 5;
        static double[] PrevFrames = new double[kFrameSkipModValue];    // Previous N frame times for FrameSkip mode.
