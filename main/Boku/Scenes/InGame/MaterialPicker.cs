...
namespace Boku
{
    ...
    /// </summary>
    public class MaterialPicker : BasePicker
    {
        ...
        public static bool FabricMode = false;
        ...
        }   // end of MaterialPicker IncrementFocus()
        ...
        private bool RayIntersectWithMaterials( Ray ray )
        {
            ...
            for( int i=0; !bIntersect && i<grid.ActualDimensions.X; ++i )
            {
                Debug.Assert( null != grid.Get(i,0) as UIGridMaterialElement);
                Matrix m = (grid.Get(i,0) as UIGridMaterialElement).HitWorldMatrix;
                bIntersect = null != ray.Intersects(new BoundingSphere(m.Translation, 2.5f * m.M33));
            }
