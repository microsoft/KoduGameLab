...
namespace Boku.Common.ParticleSystem
{
    ...
    public class ScanExplosionEmitter : BaseSpriteEmitter
    {
        ...
        // c'tor
        public ScanExplosionEmitter(ParticleSystemManager manager)
            : base(manager)
        {
        ...
        {
            // Load the texture.
            if (ScanExplosionEmitter.texture == null)
            {
                ScanExplosionEmitter.texture = BokuGame.Load<Texture2D>(BokuGame.Settings.MediaPath + @"Textures/ScanFire");
            }
        ...
        {
            ScanExplosionEmitter.texture = null;
    ...
    // sprites which fade and grow as they age.  The sprites
    // are rendered using additive blending.
    public class ScanSmokeEmitter : BaseSpriteEmitter
    {
        private static Texture2D texture = null;
        ...
        // c'tor
        public ScanSmokeEmitter(ParticleSystemManager manager)
            : base(manager)
        {
            Color = new Vector4(1.0f, 1.0f, 1.0f, 1.0f);
            StartRadius = 0.0f;
        ...
        {
            // Load the texture.
            if (ScanSmokeEmitter.texture == null)
            {
                ScanSmokeEmitter.texture = BokuGame.Load<Texture2D>(BokuGame.Settings.MediaPath + @"Textures/ScanSmoke");
            }
        }   // end of ExplosionEmitter LoadContent()
